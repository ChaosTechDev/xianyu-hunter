...
# src.app 的导入链会经过 src.api.routes.accounts -> account_check_service，
# 该模块顶层 import playwright。缺少 playwright 时整文件跳过，保留用例。
pytest.importorskip("playwright", reason="缺少 playwright 依赖，跳过应用 lifespan 用例")
