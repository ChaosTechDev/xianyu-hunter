...
# .env 是唯一配置真源，必须 override=True。
# 默认的 load_dotenv() 不覆盖已存在的进程环境变量，而 docker-compose 的
# environment 块会注入占位符（如 OPENAI_API_KEY=""），导致用户在设置页写入
# config/.env 的真实配置被静默覆盖 —— 实测会让 API_KEY 变成空串、AI 客户端
# 初始化失败，而 Web 层（env_manager 优先读 .env）仍显示"已配置"，
# 形成"界面显示 A、实际用 B"的错配。
