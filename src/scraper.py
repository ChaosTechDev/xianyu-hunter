...
from src.services.search_filter_injection import build_route_handler
...
from src.services.watch_service import finalize_watch_scan, process_watch_snapshots
...
async def _notify_task_completed(task_config: dict, item_count: int, error: str | None) -> None:
    """任务采集完成通知：输出采集结果统计（成功/失败都推送）。"""
    try:
        from src.services.notification_service import build_notification_service

        service = build_notification_service()
        task_name = task_config.get("task_name", "任务")
        keyword = task_config.get("keyword", "")
        status = "失败" if error else "完成"
        payload = {
            "商品标题": f"[任务{status}] {task_name}",
            "当前售价": f"新采集 {item_count} 件",
            "商品链接": "#",
            "通知标题": f"✅ 任务{status}：{task_name}",
        }
        reason = f"关键词: {keyword}；本轮采集新商品 {item_count} 件。"
        if error:
            reason += f"失败原因: {error[:200]}"
        await service.send_notification(payload, reason)
        print(f"   -> 任务完成通知已发送: {task_name}")
    except Exception as exc:
        print(f"   -> 任务完成通知发送失败: {exc}")


def _extract_core_keyword(keyword: str) -> str:
    """从任务关键词提取核心词（最长的字母数字 token，如 3080、5060ti）"""
    import re
    tokens = re.findall(r"[A-Za-z0-9]{3,}", keyword or "")
    if not tokens:
        return ""
...
async def _persist_login_state(context, state_file: str, original_snapshot) -> bool:
    """Atomically persist refreshed cookies without discarding snapshot metadata."""
    try:
        refreshed = await context.storage_state()
        payload = refreshed
        if isinstance(original_snapshot, dict) and any(
            key in original_snapshot for key in ("env", "headers", "page", "storage")
        ):
            payload = dict(original_snapshot)
            payload["cookies"] = refreshed.get("cookies", [])
            payload["origins"] = refreshed.get("origins", [])
        ...
        print(f"登录态自动回写失败: {type(exc).__name__}: {exc}")
        return False
...
#: 浏览器按「请求类型」自动生成的 Fetch Metadata 头，绝不能被统一覆盖。
#:
#: 增强快照是从某一个请求上抓的，存的自然是那一次的值。实测 `state/xy-1.json`
#: 里是 ``Sec-Fetch-Mode: cors`` / ``Sec-Fetch-Dest: empty`` /
#: ``Sec-Fetch-Site: same-origin`` —— 那是 XHR 的值。一旦作为全局
#: ``extra_http_headers`` 下发，**主文档导航**也会带上 cors/empty/same-origin，
#: 而导航本应是 navigate/document/请求来源。服务端据此判定为异常请求，
#: 返回一个没有任何内容的 SPA 空壳：
#:   真机实测（NAS 容器，同一 IP、同一登录态，逐个变量对照）
#:     不设 extra_http_headers          -> 命中搜索接口，正文 4972 字、html 263 KB
#:     单独加这三个中任意一个           -> 未命中，正文 0 字、html 仅 10558 B
#:     单独加 UA/sec-ch-ua/Accept/
#:       Referer/Accept-Encoding 等     -> 仍命中
#: 因此只排除 Sec-Fetch-* 系列即可，其余头保留（它们对指纹一致性有用）。
_FETCH_METADATA_HEADERS = frozenset(
...
def _build_extra_headers(raw_headers: Optional[dict]) -> dict:
    if not raw_headers:
        return {}
    excluded = {"cookie", "content-length"} | _FETCH_METADATA_HEADERS
    headers = {}
...
async def scrape_xianyu(task_config: dict, debug_limit: int = 0):
    ...
    # Automatic mode keeps the legacy root state as a candidate but can fall
    # back to the shared account directory when that state expires.
    if runtime_plan["prefer_root_state"]:
        account_candidates = [STATE_FILE, *account_items]
    elif runtime_plan["use_account_pool"]:
        account_candidates = account_items
    else:
        account_candidates = [forced_account] if forced_account else []
    account_candidates = list(dict.fromkeys(path for path in account_candidates if path))
    rotation_settings["account_enabled"] = bool(account_candidates)
    account_owner = f"{task_config.get('task_name', 'task')}:{os.getpid()}:{history_run_id}"
    ...
    def _select_account(force_new: bool = False) -> Optional[RotationItem]:
        ...
        if force_new and selected_account:
            excluded_accounts.add(selected_account.value)
            release_account(account_owner)
        if forced_account:
            picked = reserve_account(
                [forced_account],
                owner=account_owner,
                lease_seconds=max(300, rotation_settings["account_blacklist_ttl"] * 2),
                include_unhealthy=True,
            )
        elif not rotation_settings["account_enabled"]:
            picked = None
        else:
            candidates = [path for path in account_candidates if path not in excluded_accounts]
            picked = reserve_account(
                candidates,
                owner=account_owner,
                lease_seconds=max(300, rotation_settings["account_blacklist_ttl"] * 2),
        ...
        if not picked and selected_account and not force_new:
            picked = selected_account.value
        if not picked:
            return None
        selected_account = RotationItem(value=picked)
    ...
    async def _run_scrape_attempt(state_file: str, proxy_server: Optional[str]) -> int:
        ...
        # RUN_HEADLESS=false 时容器内没有 XServer，浏览器会因
        # "Missing X server or $DISPLAY" 直接起不来，这里按需拉起 Xvfb。
        #
        # 必须在 ``async_playwright()`` **之前**调用：进入该上下文管理器会立刻
        # 启动 Playwright 的 Node 驱动进程，驱动按当时的环境派生子进程。
        # 之后再设 os.environ["DISPLAY"] 驱动进程已经看不到了，实测表现为
        # 日志里「已启动虚拟显示 Xvfb」紧接着又报 Missing X server。
        ...
        async with async_playwright() as p:
            ...
            # 原生筛选注入：拦截浏览器自己发出的搜索 POST，把筛选合并进 data 并重算签名。
            # 这样筛选不再依赖 DOM 文案/哈希类名（原点击式路径在主报告 issue #1 里失效）。
            # 注入是纯优化：失败即放行原请求，不影响采集可用性。
            try:
                await page.route(
                    "**/h5/mtop.taobao.idlemtopsearch.pc.search/**",
                    build_route_handler(
            ...
            except Exception as exc:  # noqa: BLE001 — 装不上 handler 也要继续采集
                print(f"LOG: 原生筛选注入未启用（不影响采集）：{type(exc).__name__}: {exc}")
            ...
            try:
                ...
                async with page.expect_response(
                    is_search_results_response, timeout=60000
                    ...
                    try:
                        ...
                        if await area_trigger.count():
                            ...
                            async def _click_in_column(
                                ...
                                # 选择器层层降级。闲鱼的类名带构建期哈希后缀
                                # （如 provItem--QAdOx8nD），一旦前端重新构建就会失效。
                                # 因此按「哈希类名 → 类名前缀 → 纯文本」顺序尝试：
                                # 前两个靠结构，最后一个是兜底，即使类名全变也能点到。
                                selectors = (
                                    ".provItem--QAdOx8nD",
                                    "[class*='provItem']",
                                )
                                ...
                                for selector in selectors:
                                    candidate = column_locator.locator(
                                        selector, has_text=text_value
                                    ).first
                                    if await candidate.count():
                                        option = candidate
                                        break
                                if option is None:
                                    # 文本兜底：取该列内精确匹配该文本的元素本身
                                    fallback = column_locator.get_by_text(
                                        text_value, exact=True
                                    ).first
                                    if await fallback.count():
                                        option = fallback

                                if option is not None and await option.count():
                                    await option.click()
                                    await random_sleep(1.5, 2)
                                    try:
                ...
                for page_num in range(1, max_pages + 1):
                    ...
                    await process_watch_snapshots(new_snapshots)
                    ...
                    for i, item_data in enumerate(basic_items, 1):
                        ...
                        try:
                            ...
                            if detail_response.ok:
                                ...
                                # 严格关键词匹配：标题不含关键词核心词的跳过（防止脏数据污染）
                                if task_config.get("strict_keyword_match", True):
                                    _core = _extract_core_keyword(keyword)
                                    _item_title = item_data.get("商品标题") or ""
                                    if _core and _core.lower() not in _item_title.lower():
                                        print("   -> 标题不含关键词 '" + _core + "'，跳过: " + _item_title[:40])
                                        continue
                ...
                await _persist_login_state(context, state_file, snapshot_data)
            finally:
    ...
    # 单轮关注列表探活配额。探活要起浏览器页面，成本高且会增加风控暴露，
    # 因此设上限；用完则回退到「连续缺失」兜底判定。可用环境变量调节。
    try:
        liveness_max_probes = int(os.getenv("WATCH_LIVENESS_MAX_PROBES", "20"))
    except (TypeError, ValueError):
        liveness_max_probes = 20
    ...
    for attempt in range(1, attempt_limit + 1):
        ...
        try:
            ...
            if not debug_limit:
                # 判死前先做权威探活（详情接口 ret 码），避免把「这轮没采到」
                # 的在售商品误判为下架。配额与限速由 LivenessProbe 内部管理；
                # 探活失败一律保守判活，不影响主流程。
                probe = LivenessProbe(
                    storage_state=state_path,
                    max_probes=liveness_max_probes,
                )
                await finalize_watch_scan(
                    task_name=task_name_for_guard,
                    seen_item_ids=history_seen_item_ids,
                    run_id=history_run_id,
                    liveness_probe=probe,
                )
                if probe.probes_used:
                    print(f"[关注列表] 本轮完成 {probe.probes_used} 次权威探活")
            last_error = ""
            FAILURE_GUARD.record_success(task_name_for_guard)
            record_account_success(state_path)
            break
            ...
            rotate_account_next = True
            if attempt < attempt_limit and not forced_account:
                print("当前账号登录态已失效，将切换下一个账号重试。")
                continue
            break
            ...
            record_account_failure(
                state_path,
                last_error,
                cooldown_seconds=rotation_settings["account_blacklist_ttl"],
            )
            rotate_account_next = (
                rotation_settings["account_mode"] == "on_failure"
    ...
    # 任务完成通知（成功/失败都发，附采集统计）
    try:
        await _notify_task_completed(task_config, processed_item_count, last_error)
    except Exception:
        pass
