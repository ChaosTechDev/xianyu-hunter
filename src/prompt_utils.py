...
#: 生成分析标准时允许的最大输出 token。
#:
#: 不能沿用早先的 800：那个值是按**非推理模型**估的，只够装正文。DeepSeek
#: v4.1-flash 这类**推理模型**会先产出一大段 `reasoning_content`，而它同样
#: 计入 max_tokens。同一个提示词下实测：
#:   max_tokens=800  -> finish_reason=length，正文长度 0
#:   max_tokens=2000 -> finish_reason=length，正文长度 0（思考占满 2000）
#:   max_tokens=4000 -> finish_reason=length，正文长度 0（思考占满 4000）
#:   max_tokens=8000 -> finish_reason=stop，正文 1983 字，型号正确
#: 预算不足时的表现不是「报错」而是「内容静默为空」，极难排查。
#: 因此取 8192（实测可用的那一档）；这只是上限，实际生成约消耗 4000。
GENERATE_CRITERIA_MAX_TOKENS = 8192
...
async def _request_generated_text(ai_client: AIClient, prompt: str) -> str:
    ...
    try:
        generated_text = await ai_client._call_ai(
            ...
            temperature=0.5,
            max_output_tokens=GENERATE_CRITERIA_MAX_TOKENS,
