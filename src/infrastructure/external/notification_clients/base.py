...
class NotificationClient(ABC):
    ...
    def _build_message(self, product_data: Dict, reason: str) -> NotificationMessage:
        ...
        notification_title = product_data.get(
            "通知标题", f"🚨 新推荐! {short_title}{suffix}"
        )

        info = product_data.get('商品信息') or {}
        main_image = product_data.get('商品主图链接') or info.get('商品主图链接')
        if not main_image:
            image_list = info.get('商品图片列表') or product_data.get('商品图片列表', [])
            if image_list:
