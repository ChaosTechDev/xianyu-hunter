...
def _load_json_file(path: Path):
    """读取并解析 JSON 配置，任何异常都降级为 None。

    这里必须容错，不能用裸 ``read_text`` + ``json.loads``：本函数位于
    ``src/app.py`` lifespan 的 ``bootstrap_sqlite_storage()`` 调用链上，
    一旦抛出异常应用会**启动失败**，用户升级后再也进不去网页、连改回配置的
    界面都没有。

    已知会遇到的两种情况：
    - ``IsADirectoryError``：docker compose 的 bind mount 源文件不存在时，
      Docker 会把它创建成**目录**（本机实测复现：config.json 被建成目录，
      容器启动即 ``IsADirectoryError: [Errno 21] Is a directory``）。
    - ``JSONDecodeError``：用户手改 legacy 配置写坏了。

    两种情况都返回 None，让调用方走「无 legacy 任务」的分支，应用照常启动。
    """
    try:
        ...
        # OSError 覆盖 IsADirectoryError/PermissionError/UnicodeDecodeError 之外
        # 的 IO 问题；ValueError 覆盖 json.JSONDecodeError
        return None
    ...
    for index, raw_task in enumerate(tasks):
        ...
        conn.execute(
            ...
            INSERT INTO tasks (
                ...
                max_pages, personal_only, min_price, max_price, notify_price_below, auto_consult, cron,
            ...
            (
                ...
                # notify_price_below / auto_consult：legacy config.json 里通常没有，
                # 缺失时留空由 schema 默认值兜底（不能错位挤占 cron 的位置）
                raw_task.get("notify_price_below"),
                _as_int(raw_task.get("auto_consult", False)),
                raw_task.get("cron"),
...
def _parse_price(value):
    ...
    委托给 ``price_history_service.parse_price_value``，避免同源逻辑存在两份副本
    —— 历史上这里和那边各写了一遍，于是同一个 bug（``"万"`` 后缀抛未捕获异常、
    ``nan``/``inf`` 无防护）在修一处时另一处会漏。现在只有唯一实现。
    """
