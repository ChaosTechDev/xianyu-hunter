...
def _parse_state(raw: str, path: str) -> dict:
    """解析状态文件内容；只有**非空但不可解析**的内容才算损坏。

    空文件是合法的「尚未写入过状态」：``_update_task`` 以 ``a+`` 打开文件时
    会先创建一个 0 字节文件，随即读到的就是空内容。若把它当成损坏，就会在
    写入之前把文件改名成 ``*.corrupt.<ts>``，导致后续写入落进已被改名的
    inode，``path`` 上什么都不剩 —— 表现为失败计数永远停在 0、熔断器永不
    触发。因此这里必须区分「空」与「损坏」。
    """
    if not raw.strip():
        return {}
    try:
        ...
        try:
            ...
            os.replace(path, "%s.corrupt.%s" % (path, ts))
        except Exception:
            pass
        return {}
...
class FailureGuard:
    ...
    def _update_task(self, task_key: str, updater) -> dict:
        ...
        with open(self.path, "a+", encoding="utf-8") as fh:
            with _FileLock(fh):
                # 从已锁定的句柄本身读取，而不是重新按路径打开：句柄在锁内，
                # 内容才与随后的写入属于同一个文件版本。
                fh.seek(0)
                data = _parse_state(fh.read(), self.path)
                if "tasks" not in data or not isinstance(data.get("tasks"), dict):
                    data = {"version": 1, "tasks": {}}
                data.setdefault("version", 1)
                ...
                # Windows does not allow os.replace() while the destination is
                # open and locked. Persist through the locked handle so updates
                # remain serialized on every supported platform.
                fh.seek(0)
                fh.truncate(0)
