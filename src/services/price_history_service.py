...
def parse_price_value(value: Any) -> Optional[float]:
    """把各种价格写法解析成有限的两位小数浮点，解析不出来时返回 None。

    容错要点（两处都踩过坑）：
    - ``nan`` / ``inf`` / ``1e400`` 能被 ``float()`` 成功解析，但落库后会让
      min/max/avg 变成 inf/nan，``price <= alert_price`` 之类的比较也会失真。
      因此统一用 ``math.isfinite`` 挡掉。
    - ``"万"`` 后缀的换算必须包在 try 里：``"万"`` / ``"abc万"`` / ``"面议万"``
      会让 ``float(text[:-1])`` 抛 ValueError。本函数有 20+ 个调用点，多数没有
      try 包裹，一旦抛出会中断整批采集处理，所以必须在此处兜住。
    """
    if value is None:
        return None
    if isinstance(value, (int, float)):
        # 注意 bool 是 int 子类，会走这个分支（True -> 1.0）。这是既有契约，
        # 有意保留：闲鱼价格字段不会出现布尔值，收紧它没有收益却可能影响调用方。
        number = float(value)
