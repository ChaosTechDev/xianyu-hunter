...
#: 前端以开关形式提交的布尔字段。必须在这里登记，否则
#: ``_normalize_patch_value`` 会把 ``False`` 变成字符串 ``"False"``，
#: 而 ``model_construct`` 不做类型转换，最终 ``bool("False")`` 为真——开关关不掉。
BOOLEAN_NOTIFICATION_FIELDS = {"PCURL_TO_MOBILE", "SMTP_USE_SSL", "SMTP_USE_STARTTLS"}

#: 需要转成整数的字段。同上：字符串形式能进库但在比较处容易出错。
INTEGER_NOTIFICATION_FIELDS = {"SMTP_PORT"}
...
def build_notification_settings_response(
    ...
    response = {
        ...
        "SMTP_PASSWORD": "",
        "MAIL_FROM": notification_settings.mail_from or "",
        "MAIL_TO": notification_settings.mail_to or "",
        "SMTP_USE_SSL": notification_settings.smtp_use_ssl,
        "SMTP_USE_STARTTLS": notification_settings.smtp_use_starttls,
...
def build_notification_status_flags(
    ...
    return {
        ...
        "mail_to_set": bool(notification_settings.mail_to),
    }
...
def build_configured_channels(
    ...
    # 邮件渠道的启用条件与 EmailClient 保持一致：服务器 + 收件人。
    # 两处判据必须相同，否则会出现「状态显示已配置但实际发不出去」。
    if notification_settings.smtp_host and notification_settings.mail_to:
    ...
    return _build_notification_settings_model(
        {
            ...
            "smtp_port": _env_int_or_none(env_manager.get_value("SMTP_PORT")),
            "smtp_user": _normalize_existing_text(env_manager.get_value("SMTP_USER")),
            "smtp_password": _normalize_existing_text(env_manager.get_value("SMTP_PASSWORD")),
            "mail_from": _normalize_existing_text(env_manager.get_value("MAIL_FROM")),
            "mail_to": _normalize_existing_text(env_manager.get_value("MAIL_TO")),
            "smtp_use_ssl": _env_bool(env_manager.get_value("SMTP_USE_SSL"), True),
            "smtp_use_starttls": _env_bool(env_manager.get_value("SMTP_USE_STARTTLS"), False),
...
def _normalize_patch_value(env_name: str, value):
    if env_name in BOOLEAN_NOTIFICATION_FIELDS:
        # 前端可能传 bool，也可能传 "true"/"false" 字符串
        if isinstance(value, str):
            return value.strip().lower() in {"1", "true", "yes", "y", "on"}
        return bool(value)
    if env_name in INTEGER_NOTIFICATION_FIELDS:
...
def _env_int_or_none(value: str | None) -> int | None:
    """读取可选的整数配置。空值与非法值都返回 None（交由下游取默认端口）。"""
    if value is None:
        return None
    text = str(value).strip()
    if not text:
