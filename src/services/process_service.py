...
class ProcessService:
    ...
    def __init__(self):
        ...
        self.paused_task_ids: set[int] = set()
        self.manual_stop_task_ids: set[int] = set()
    ...
    def is_paused(self, task_id: int) -> bool:
        return task_id in self.paused_task_ids and self.is_running(task_id)
    ...
    def _register_runtime(
        ...
        self.paused_task_ids.discard(task_id)
    ...
    async def stop_task(self, task_id: int) -> bool:
        ...
        try:
            self.manual_stop_task_ids.add(task_id)
            if self.is_paused(task_id) and sys.platform != "win32":
            ...
            self.manual_stop_task_ids.discard(task_id)

    async def pause_task(self, task_id: int) -> bool:
        process = self.processes.get(task_id)
        if process is None or process.returncode is not None or self.is_paused(task_id):
            return False
        if sys.platform == "win32":
            raise RuntimeError("任务暂停需要在 Docker/Linux 环境运行")
        os.killpg(os.getpgid(process.pid), signal.SIGSTOP)
        self.paused_task_ids.add(task_id)
        return True

    async def resume_task(self, task_id: int) -> bool:
        process = self.processes.get(task_id)
        if process is None or process.returncode is not None or not self.is_paused(task_id):
            return False
        if sys.platform == "win32":
            raise RuntimeError("任务继续需要在 Docker/Linux 环境运行")
        os.killpg(os.getpgid(process.pid), signal.SIGCONT)
        self.paused_task_ids.discard(task_id)
        return True
