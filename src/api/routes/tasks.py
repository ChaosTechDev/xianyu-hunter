...
async def start_task(
    ...
    updated = await task_service.update_task(
        task_id, TaskUpdate(is_running=True, runtime_status="running", last_failure_reason=None)
    )
    return {"message": f"任务 '{task.task_name}' 已启动", "task": updated.model_dump(mode="json")}
...
async def stop_task(
    ...
    await process_service.stop_task(task_id)
    updated = await task_service.update_task(
        task_id, TaskUpdate(is_running=False, runtime_status="stopped")
    )
    return {"message": f"任务ID {task_id} 已发送停止信号", "task": updated.model_dump(mode="json")}


@router.post("/{task_id}/run-once", response_model=dict)
async def run_task_once(
    task_id: int,
    task_service: TaskService = Depends(get_task_service),
    process_service: ProcessService = Depends(get_process_service),
):
    return await start_task(task_id, task_service, process_service)
...
async def pause_task(
    ...
    if not await task_service.get_task(task_id):
        raise HTTPException(status_code=404, detail="任务未找到")
    try:
        success = await process_service.pause_task(task_id)
    except RuntimeError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if not success:
        raise HTTPException(status_code=409, detail="任务当前不可暂停")
    updated = await task_service.update_task(
        task_id, TaskUpdate(is_running=True, runtime_status="paused")
    )
    return {"message": "任务已暂停", "task": updated.model_dump(mode="json")}
...
async def resume_task(
    ...
    if not await task_service.get_task(task_id):
        raise HTTPException(status_code=404, detail="任务未找到")
    try:
        success = await process_service.resume_task(task_id)
    except RuntimeError as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc
    if not success:
        raise HTTPException(status_code=409, detail="任务当前不可继续")
    updated = await task_service.update_task(
        task_id, TaskUpdate(is_running=True, runtime_status="running")
    )
    return {"message": "任务已继续", "task": updated.model_dump(mode="json")}
