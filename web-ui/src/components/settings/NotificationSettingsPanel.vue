...
const channelFields: Record<ChannelKey, (keyof NotificationSettingsUpdate)[]> = {
  ...
  email: ['SMTP_HOST', 'SMTP_PORT', 'SMTP_USER', 'SMTP_PASSWORD', 'MAIL_FROM', 'MAIL_TO', 'SMTP_USE_SSL', 'SMTP_USE_STARTTLS'],
}
...
function syncFromSettings(settings: NotificationSettings) {
  ...
  initialValues.SMTP_PORT = settings.SMTP_PORT ?? null
  initialValues.SMTP_USER = settings.SMTP_USER ?? ''
  initialValues.MAIL_FROM = settings.MAIL_FROM ?? ''
  initialValues.MAIL_TO = settings.MAIL_TO ?? ''
  initialValues.SMTP_USE_SSL = settings.SMTP_USE_SSL ?? true
  initialValues.SMTP_USE_STARTTLS = settings.SMTP_USE_STARTTLS ?? false
...
// Switch 的 v-model 给的是真布尔。不能用 updateField，那里会把值转成字符串，
// 于是 "false" 既与初始布尔值永不相等（每次都误判为"已修改"），
// 又让最终提交的 !!form[field] 恒为 true —— 开关关不掉。
function updateBooleanField(field: keyof NotificationSettingsUpdate, value: boolean) {
...
function clearChannel(channel: ChannelKey) {
  ...
  if (channel === 'email') {
    // 端口留空交给后端按加密方式取默认值（SSL 465 / STARTTLS 587），
    // 这里必须清成 null 而不是 ''，否则会被当成非法整数。
    form.SMTP_PORT = null
...
function buildScopedPayload(channel?: ChannelKey): NotificationSettingsUpdate {
  ...
  const mutablePayload = payload as Record<string, string | boolean | number | null | undefined>
  ...
  const textFields: (keyof NotificationSettingsUpdate)[] = [
    ...
    'SMTP_HOST', 'SMTP_USER', 'MAIL_FROM', 'MAIL_TO',
  ]
  const booleanFields: (keyof NotificationSettingsUpdate)[] = [
  ...
  // 端口是数字字段：空值传 null（后端回落默认端口），非空转 number。
  // 不能走 textFields —— 那里会把数字变成字符串，后端虽然能解析，
  // 但类型不一致会在比较处埋雷（'' > 0 之类）。
  if (!includedFields || includedFields.has('SMTP_PORT')) {
    if (mutableClearedFields.SMTP_PORT) {
      mutablePayload.SMTP_PORT = null
    } else {
      const rawPort = String(mutableForm.SMTP_PORT ?? '').trim()
      const initialPort = String(mutableInitialValues.SMTP_PORT ?? '').trim()
      if (rawPort !== initialPort) {
        mutablePayload.SMTP_PORT = rawPort ? Number(rawPort) : null
      }
    }
  }
  ...
  for (const field of booleanFields) {
    ...
    if (form[field] !== initialValues[field]) {
      mutablePayload[field as string] = !!form[field]
    }
  }
...
<template>
  <div class="space-y-4">
    ...
    <div v-else class="grid gap-4">
      ...
      <Card class="app-surface-subtle overflow-hidden border-l-4 border-l-sky-500">
        <CardHeader><CardTitle class="flex items-center gap-2"><Mail class="h-4 w-4 text-sky-500" /> {{ t('notifyPanel.email.title') }}</CardTitle><CardDescription>{{ t('notifyPanel.email.description') }}</CardDescription></CardHeader>
        <CardContent class="grid gap-4">
          <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.hostLabel') }}</Label><Input :model-value="form.SMTP_HOST ?? ''" :placeholder="t('notifyPanel.email.hostPlaceholder')" @update:model-value="(value) => updateField('SMTP_HOST', String(value))" /></div>
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.portLabel') }}</Label><Input :model-value="form.SMTP_PORT ?? ''" type="number" :placeholder="t('notifyPanel.email.portPlaceholder')" @update:model-value="(value) => updateField('SMTP_PORT', String(value))" /></div>
          </div>
          <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.userLabel') }}</Label><Input :model-value="form.SMTP_USER ?? ''" :placeholder="t('notifyPanel.email.userPlaceholder')" @update:model-value="(value) => updateField('SMTP_USER', String(value))" /></div>
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.passwordLabel') }}</Label><Input :model-value="form.SMTP_PASSWORD ?? ''" type="password" :placeholder="t('notifyPanel.secretPlaceholder')" @update:model-value="(value) => updateSecretField('SMTP_PASSWORD', String(value))" /></div>
          </div>
          <div class="grid gap-4 md:grid-cols-2">
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.fromLabel') }}</Label><Input :model-value="form.MAIL_FROM ?? ''" :placeholder="t('notifyPanel.email.fromPlaceholder')" @update:model-value="(value) => updateField('MAIL_FROM', String(value))" /></div>
            <div class="grid gap-2"><Label>{{ t('notifyPanel.email.toLabel') }}</Label><Input :model-value="form.MAIL_TO ?? ''" :placeholder="t('notifyPanel.email.toPlaceholder')" @update:model-value="(value) => updateField('MAIL_TO', String(value))" /></div>
          </div>
          <div class="grid gap-4 md:grid-cols-2">
            <div class="flex items-center justify-between gap-3 rounded-2xl border px-4 py-3">
              <div class="grid gap-1">
                <Label for="smtp-use-ssl">{{ t('notifyPanel.email.useSslLabel') }}</Label>
                <p class="text-xs text-slate-500">{{ t('notifyPanel.email.useSslHelp') }}</p>
              </div>
              <Switch id="smtp-use-ssl" :model-value="!!form.SMTP_USE_SSL" @update:model-value="(value) => updateBooleanField('SMTP_USE_SSL', !!value)" />
            </div>
            <div class="flex items-center justify-between gap-3 rounded-2xl border px-4 py-3">
              <div class="grid gap-1">
                <Label for="smtp-use-starttls">{{ t('notifyPanel.email.useStartTlsLabel') }}</Label>
                <p class="text-xs text-slate-500">{{ t('notifyPanel.email.useStartTlsHelp') }}</p>
              </div>
              <Switch id="smtp-use-starttls" :model-value="!!form.SMTP_USE_STARTTLS" @update:model-value="(value) => updateBooleanField('SMTP_USE_STARTTLS', !!value)" />
            </div>
          </div>
          <div class="rounded-2xl border border-dashed border-sky-200 bg-sky-50/70 px-4 py-3 text-sm text-sky-700">
            <p>{{ t('notifyPanel.email.help') }}</p>
          </div>
        </CardContent>
